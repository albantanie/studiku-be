...
func (r *Repository) Dashboard() (map[string]interface{}, error) {
	...
	for i, a := range assignments {
		if len(deadlines) == 3 { break }
		if a.Status == "pending" { deadlines = append(deadlines, models.DashboardDeadline{ID: a.ID, Title: a.Title, Course: a.Course, DueDate: a.DueDate, Urgent: i == 0}) }
	}
...
func (r *Repository) StudentCourses() ([]models.StudentCourse, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&c.ID, &c.Name, &c.Lecturer, &c.Tutor, &c.Schedule.Day, &c.Schedule.StartTime, &c.Schedule.EndTime, &c.Attendance.Present, &c.Attendance.Total, &c.Color); err != nil { return nil, err }
		if c.Attendance.Total > 0 { c.Attendance.Percentage = c.Attendance.Present * 100 / c.Attendance.Total }
...
func (r *Repository) Assignments() ([]models.Assignment, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&a.ID,&a.Title,&a.Course,&a.Assistant,&a.DueDate,&a.DueTime,&a.Status,&a.CourseColor,&a.SubmittedDate,&a.Score); err != nil { return nil, err }
...
func (r *Repository) AdminCourses() ([]models.AdminCourse, error) {
	...
	for rows.Next() { var c models.AdminCourse; if err := rows.Scan(&c.ID,&c.Name,&c.Instructor,&c.Assistant,&c.StudyProgram,&c.AcademicYear,&c.ClassCode,&c.Status,&c.Day,&c.StartTime,&c.EndTime,&c.Room,&c.Sessions,&c.Credits,&c.Students); err != nil { return nil, err }; items = append(items, c) }
	return items, rows.Err()
}
...
func (r *Repository) UpdateCourse(id int, c *models.AdminCourse) error { _, err := r.db.Exec(`UPDATE courses SET name=$1,instructor=$2,assistant=$3,study_program=$4,academic_year=$5,class_code=$6,status=$7,day=$8,start_time=$9,end_time=$10,room=$11,sessions=$12,credits=$13,students=$14,updated_at=now() WHERE id=$15`, c.Name,c.Instructor,c.Assistant,c.StudyProgram,c.AcademicYear,c.ClassCode,c.Status,c.Day,c.StartTime,c.EndTime,c.Room,c.Sessions,c.Credits,c.Students,id); return err }
func (r *Repository) DeleteCourse(id int) error { _, err := r.db.Exec(`DELETE FROM courses WHERE id=$1`, id); return err }
...
func (r *Repository) AcademicYears() ([]models.AcademicYear, error) {
	...
	for rows.Next() { var y models.AcademicYear; if err := rows.Scan(&y.ID,&y.Name,&y.StartDate,&y.EndDate,&y.Semester,&y.Status,&y.TotalCourses,&y.TotalStudents); err != nil { return nil, err }; items = append(items,y) }
	return items, rows.Err()
}
func (r *Repository) CreateAcademicYear(y *models.AcademicYear) error { return r.db.QueryRow(`INSERT INTO academic_years (name,start_date,end_date,semester,status,total_courses,total_students) VALUES ($1,$2,$3,$4,COALESCE(NULLIF($5,''),'Mendatang'),$6,$7) RETURNING id`, y.Name,y.StartDate,y.EndDate,y.Semester,y.Status,y.TotalCourses,y.TotalStudents).Scan(&y.ID) }
func (r *Repository) UpdateAcademicYear(id int, y *models.AcademicYear) error { _, err := r.db.Exec(`UPDATE academic_years SET name=$1,start_date=$2,end_date=$3,semester=$4,status=$5,total_courses=$6,total_students=$7,updated_at=now() WHERE id=$8`, y.Name,y.StartDate,y.EndDate,y.Semester,y.Status,y.TotalCourses,y.TotalStudents,id); return err }
func (r *Repository) DeleteAcademicYear(id int) error { _, err := r.db.Exec(`DELETE FROM academic_years WHERE id=$1`, id); return err }
...
func (r *Repository) Students() ([]models.Student, error) {
	...
	for rows.Next() { var s models.Student; if err := rows.Scan(&s.ID,&s.Name,&s.Email,&s.Password,&s.DefaultPassword,&s.StudentID,&s.Program,&s.Semester,pq.Array(&s.Courses),&s.Status,&s.JoinDate,&s.IsPasswordChanged); err != nil { return nil, err }; items = append(items,s) }
	return items, rows.Err()
}
func (r *Repository) CreateStudent(s *models.Student) error { if s.DefaultPassword == "" { s.DefaultPassword = "password" }; if s.Password == "" { s.Password = "password" }; return r.db.QueryRow(`INSERT INTO students (name,email,password,default_password,student_id,program,semester,courses,status,join_date,is_password_changed) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,COALESCE(NULLIF($9,''),'Aktif'),COALESCE(NULLIF($10,'')::date,CURRENT_DATE),$11) RETURNING id`, s.Name,s.Email,s.Password,s.DefaultPassword,s.StudentID,s.Program,s.Semester,pq.Array(s.Courses),s.Status,s.JoinDate,s.IsPasswordChanged).Scan(&s.ID) }
func (r *Repository) UpdateStudent(id int, s *models.Student) error { _, err := r.db.Exec(`UPDATE students SET name=$1,email=$2,password=$3,default_password=$4,student_id=$5,program=$6,semester=$7,courses=$8,status=$9,join_date=$10,is_password_changed=$11,updated_at=now() WHERE id=$12`, s.Name,s.Email,s.Password,s.DefaultPassword,s.StudentID,s.Program,s.Semester,pq.Array(s.Courses),s.Status,s.JoinDate,s.IsPasswordChanged,id); return err }
func (r *Repository) DeleteStudent(id int) error { _, err := r.db.Exec(`DELETE FROM students WHERE id=$1`, id); return err }
...
func (r *Repository) Lecturers() ([]models.Lecturer, error) {
	...
	if err != nil { return nil, err }
	defer rows.Close(); items := []models.Lecturer{}
	for rows.Next() { var x models.Lecturer; if err := rows.Scan(&x.ID,&x.Name,&x.Email,&x.Password,&x.DefaultPassword,&x.NIDN,pq.Array(&x.Courses),&x.IsPasswordChanged); err != nil { return nil, err }; items=append(items,x) }
	return items, rows.Err()
}
func (r *Repository) CreateLecturer(x *models.Lecturer) error {
	if x.DefaultPassword == "" { x.DefaultPassword = "password" }
	if x.Password == "" { x.Password = x.DefaultPassword }
...
func (r *Repository) DeleteLecturer(id int) error { _, err := r.db.Exec(`DELETE FROM lecturers WHERE id=$1`, id); return err }
...
func (r *Repository) Assistants() ([]models.LabAssistant, error) {
	...
	if err != nil { return nil, err }
	defer rows.Close(); items := []models.LabAssistant{}
	for rows.Next() { var x models.LabAssistant; if err := rows.Scan(&x.ID,&x.Name,&x.Email,&x.Phone,&x.StudentID,&x.Lab,&x.Supervisor,&x.Semester,&x.GPA,&x.AssignedCourses,&x.WeeklyHours,&x.Status,&x.JoinDate,&x.Password,&x.DefaultPassword,&x.IsPasswordChanged); err != nil { return nil, err }; items=append(items,x) }
	return items, rows.Err()
}
...
func (r *Repository) DeleteAssistant(id int) error { _, err := r.db.Exec(`DELETE FROM lab_assistants WHERE id=$1`, id); return err }
...
func (r *Repository) Classes() ([]models.ClassData, error) {
	...
	if err != nil { return nil, err }
	defer rows.Close(); items := []models.ClassData{}
	for rows.Next() { var c models.ClassData; if err := rows.Scan(&c.ID,&c.Code,&c.Name,&c.AcademicYear,&c.Assistant,&c.Schedule,&c.Room,&c.TotalStudents,&c.Capacity,pq.Array(&c.Students)); err != nil { return nil, err }; items=append(items,c) }
	return items, rows.Err()
}
