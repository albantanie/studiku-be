...
var pageDataQueries = map[string]string{
	...
	"admin_dashboard": `
		SELECT json_build_object(
			...
			'topCourses', COALESCE((SELECT json_agg(json_build_object('name',name,'students',students,'completion',LEAST(100,attendance_present*100/NULLIF(attendance_total,0)),'instructor',instructor) ORDER BY students DESC) FROM courses LIMIT 4), '[]'::json)
		)`,
	"lecturer_dashboard": `
		SELECT json_build_object(
			'courses', COALESCE((SELECT json_agg(json_build_object('id',id,'code',class_code,'name',name,'class',class_code,'students',students,'schedule',day || ', ' || start_time || ' - ' || end_time,'room',room,'averageGrade',78.5) ORDER BY id) FROM courses LIMIT 3), '[]'::json),
	...
	"lecturer_courses": `SELECT COALESCE(json_agg(json_build_object('id',id,'code',class_code,'name',name,'class',class_code,'semester','Ganjil','academicYear',academic_year,'students',students,'schedule',day || ', ' || start_time || ' - ' || end_time,'room',room,'sks',credits,'description','Mata kuliah ' || lower(name),'materials',(SELECT count(*) FROM course_materials WHERE course_id=courses.id),'assignments',(SELECT count(*) FROM session_assignments WHERE course_id=courses.id)) ORDER BY id), '[]'::json) FROM courses`,
	...
	"assistant_dashboard": `
		SELECT json_build_object(
			'practicalSessions', COALESCE((SELECT json_agg(json_build_object('id',id,'course',name,'class',class_name,'lab',lab,'schedule',day || ', ' || start_time || ' - ' || end_time,'students',students,'attendance',attendance_present,'topic','Praktikum ' || name) ORDER BY id) FROM assistant_practical_courses), '[]'::json),
			...
			'upcomingPracticals', COALESCE((SELECT json_agg(json_build_object('id',id,'course',name,'class',class_name,'time',day || ', ' || start_time || ' - ' || end_time,'lab',lab,'topic','Praktikum ' || name) ORDER BY id) FROM assistant_practical_courses LIMIT 2), '[]'::json),
	...
	"admin_reports": `SELECT json_build_object('stats', json_build_array(json_build_object('label','Total Mahasiswa Aktif','value',(SELECT count(*)::text FROM students WHERE status='Aktif'),'change','+12%','trend','up'), json_build_object('label','Rata-rata IPK','value','3.45','change','+0.15','trend','up'), json_build_object('label','Tingkat Kelulusan','value','87%','change','+3%','trend','up'), json_build_object('label','Mahasiswa Baru','value',(SELECT count(*)::text FROM students),'change','+8%','trend','up')), 'coursePerformance', COALESCE((SELECT json_agg(json_build_object('course',name,'students',students,'avgScore',82,'completion',attendance_present*100/NULLIF(attendance_total,0),'passing',89) ORDER BY id) FROM courses LIMIT 3), '[]'::json), 'topStudents', COALESCE((SELECT json_agg(json_build_object('name',name,'program',program,'gpa',3.75,'completedCourses',array_length(courses,1)) ORDER BY id) FROM students LIMIT 3), '[]'::json), 'monthlyRegistrations', json_build_array(json_build_object('month','Sep 2024','students',198),json_build_object('month','Oct 2024','students',156),json_build_object('month','Nov 2024','students',142),json_build_object('month','Dec 2024','students',178),json_build_object('month','Jan 2025','students',245)))`,
	"admin_assignments": `SELECT COALESCE(json_agg(json_build_object('id',a.id,'title',a.title,'course',a.course,'instructor',COALESCE(c.instructor,a.assistant),'dueDate',a.due_date,'totalStudents',COALESCE(c.students,0),'submitted',CASE WHEN a.status IN ('submitted','graded') THEN COALESCE(c.students,0) ELSE 0 END,'graded',CASE WHEN a.status='graded' THEN COALESCE(c.students,0) ELSE 0 END,'pending',CASE WHEN a.status='pending' THEN COALESCE(c.students,0) ELSE 0 END,'status',CASE WHEN a.status='graded' THEN 'Selesai' ELSE 'Aktif' END,'type',CASE WHEN lower(a.title) LIKE '%quiz%' THEN 'Quiz' WHEN lower(a.title) LIKE '%project%' THEN 'Project' ELSE 'Tugas' END) ORDER BY a.id), '[]'::json) FROM assignments a LEFT JOIN courses c ON c.name=a.course`,
